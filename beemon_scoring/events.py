...
Detection is a simple hard-floor rule: a step counts as an event when it clears
a fixed physical magnitude within a short interval and then persists. Additions
must gain at least ADDITION_FLOOR_KG (a super or feeder is an object of known
mass); harvests must drop at least HARVEST_FLOOR_PCT of colony weight and at
least HARVEST_FLOOR_KG. Ordinary foraging gains, nightly respiration loss, and
sensor jitter stay under those floors and remain inside a single segment, so a
normal week is one segment (identical to the naive behaviour).

Sister colonies at a site are handled by *corroboration as confidence*, not as
detection: each colony is detected independently, then an event is tagged
``corroborated`` when its sister also has a floor-clearing event within
CORROBORATION_WINDOW_HOURS -- regardless of direction, because a beekeeper may
harvest one hive and feed/super its sister in the same visit (equalisation).
Corroboration never lowers the detection bar and never invents an event; it only
labels how much apiary-level evidence supports one.
"""
...
class WeightEvent:
    ...
    pct_change: float
    elapsed_hours: float
    # True when the sister colony has a floor-clearing event within
    # CORROBORATION_WINDOW_HOURS -- apiary-level evidence that this is a real
    # management visit rather than an isolated anomaly. Set by _tag_corroboration
    # after independent detection; never affects whether the event is detected.
    corroborated: bool = False

    def describe(self) -> str:
        direction = "gained" if self.delta_kg > 0 else "dropped"
        support = " (corroborated)" if self.corroborated else ""
        return (
            f"Likely {self.kind}{support}: weight {direction} {abs(self.delta_kg):.1f} kg "
...
def detect_weight_events(readings: list[SensorReading]) -> list[WeightEvent]:
    ...
    callers do not have to guarantee ordering. Detection is the hard-floor rule
    described in the module docstring: each adjacent step that clears the floors
    within MAX_EVENT_INTERVAL_HOURS is a candidate; candidates are coalesced
    (so a multi-step move or a spike-then-revert is handled as one), and each
    merged event must persist to be confirmed.
    """
    ...
    for index in range(1, len(ordered)):
        ...
        if delta_kg >= ADDITION_FLOOR_KG:
            kind = "addition"
        elif delta_kg < 0 and abs(pct_change) >= HARVEST_FLOOR_PCT and abs(delta_kg) >= HARVEST_FLOOR_KG:
    ...
    # Coalesce clustered steps (drop-then-settle, dip-then-recover, or a big
    # move split across two hourly steps) into single net events FIRST, then
    # confirm each merged event actually persists. Doing it in this order means
    # a transient dip and its recovery cancel out before either half can be
    # mistaken for a standalone event.
...
def _reduce_cluster(cluster: list[WeightEvent]) -> WeightEvent | None:
    ...
    nets out to nothing meaningful (a transient that fully recovered, or a
    residual move that no longer clears the hard floors)."""
    if len(cluster) == 1:
        return cluster[0]
    ...
    # The merged move must still clear the same hard floors a standalone event
    # would, so a spike-then-partial-revert that nets out small is dropped.
    if net_delta > 0:
        if net_delta < ADDITION_FLOOR_KG:
            return None
    else:
        if abs(net_pct) < HARVEST_FLOOR_PCT or abs(net_delta) < HARVEST_FLOOR_KG:
            return None
...
def _tag_corroboration(
    events_by_side: dict[str, list[WeightEvent]],
) -> dict[str, list[WeightEvent]]:
    """Tag events whose sister colony also has a floor-clearing event nearby.

    Apiary management actions are apiary-level: a beekeeper working one hive
    usually works its neighbour in the same visit. When both colonies of a site
    each have an independently detected (floor-clearing) event within
    CORROBORATION_WINDOW_HOURS of one another, both are tagged ``corroborated``
    as high-confidence management events. This is direction-agnostic on purpose:
    a joint harvest, a joint supering, or an equalisation (harvest one side,
    feed/super the other) are all one visit.

    Confidence only. This never promotes a sub-floor move, never invents an
    event, and never removes one -- so it cannot add a false positive. Sites
    with a single colony are returned unchanged.
    """
    ...
    side_a, side_b = sides
    times_a = [event.observed_at for event in events_by_side[side_a]]
    times_b = [event.observed_at for event in events_by_side[side_b]]

    def tag(events: list[WeightEvent], sister_times: list[datetime]) -> list[WeightEvent]:
        tagged: list[WeightEvent] = []
        for event in events:
            corroborated = any(
                abs((event.observed_at - sister_time).total_seconds() / 3600) <= CORROBORATION_WINDOW_HOURS
                for sister_time in sister_times
            )
            tagged.append(replace(event, corroborated=True) if corroborated else event)
        return tagged

    return {side_a: tag(events_by_side[side_a], times_b), side_b: tag(events_by_side[side_b], times_a)}
...
def detect_site_events(readings: list[SensorReading]) -> dict[str, list[WeightEvent]]:
    ...
    tags cross-colony corroboration at site level. This is the single source of
    truth for weight events: quality filtering and feature building must both
    consume this same result so an event is exempt from data-quality checks and
    reaches the feature builder identically, instead of each stage running its
    own detection against a different slice of the data. Pure function, no I/O.
    """
    ...
    for hive_id, sides in by_hive.items():
        detected = {side: detect_weight_events(side_readings) for side, side_readings in sides.items()}
        corroborated = _tag_corroboration(detected)
